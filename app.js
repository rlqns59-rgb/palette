...
function exportExcel() {
  ...
  XLSX.writeFile(wb, '팔레트_' + new Date().toISOString().slice(0, 10) + '.xlsx');
}
...
async function addReject() {
  ...
  const date = document.getElementById('rjd').value || new Date().toISOString().slice(0, 10);
  const newRj = {
    id: uid(),
...
async function addMember() {
  ...
  const jd = document.getElementById('fjd').value || new Date().toISOString().slice(0, 10);
  const newMem = {
    id: uid(),
    name,
...
function stlCalcShares(ci) {
  ...
  const baseShare = stl.members.map((x, mi) => {
    ...
    if (nalcohol[mi]) {
      s += nalcCount > 0 ? c.drink / nalcCount : 0;
    } else {
      s += normCount > 0 ? c.liquor / normCount : 0;
      s += activeCount > 0 ? c.drink / activeCount : 0;
    }
    return s;
  });
...
/**
 * FIXED: Recalculates manual total modifications and distributes the difference among all other active sheet members.
 */
function stlEditTotal(mi, newValStr) {
  const newVal = Math.round(parseFloat(String(newValStr).replace(/[^0-9.\-]/g, '')) || 0);
  ...
  let totalCosts = stl.costs.reduce((acc, c) => acc + (c.total || 0), 0);
  let totalMoim = 0;
  let totalBj = 0;
  let totalPrepay = 0;
  
  stl.members.forEach((m) => {
    const isHost = m.name === host;
    if (!isHost && moimOn) totalMoim += 1000;
    if (!isHost && bjOn) totalBj += 1000;
    totalPrepay += (m.prepay || 0);
  });
  
  const expectedGrandTotal = totalCosts + totalMoim + totalBj - totalPrepay;
  stl.manualTotal[mi] = newVal;
  
  const others = stl.members.map((_, i) => i).filter(i => i !== mi);
  if (others.length > 0) {
    const baselines = others.map(i => {
      return stl.manualTotal[i] != null ? stl.manualTotal[i] : stlCalcAutoTotal(i);
    });
    const sumBaselines = baselines.reduce((acc, v) => acc + v, 0);
    
    // Distribute the delta evenly among the other members
    const targetOthersSum = expectedGrandTotal - newVal;
    const diffToDistribute = targetOthersSum - sumBaselines;
    
    const share = Math.floor(diffToDistribute / others.length);
    let remainder = diffToDistribute % others.length;
    
    others.forEach((i) => {
      let currentVal = stl.manualTotal[i] != null ? stl.manualTotal[i] : stlCalcAutoTotal(i);
      let adj = currentVal + share;
      if (remainder > 0) {
        adj += 1; remainder--;
      } else if (remainder < 0) {
        adj -= 1; remainder++;
      }
      stl.manualTotal[i] = adj;
    });
  }
